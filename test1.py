...
delete_status = requests.delete("{url}/treatment_plans/905tc".format(url=base_url), headers=auth)
print delete_status.text

# # Retrive treatment plan
# tp_status = requests.get("{url}/treatment_plans/5irxl".format(url=base_url),headers = auth)
# print tp_status.text

# # Post Treatment plan attempt
# treatment_plan={
# 	'data':{
...
# }
# }
# tp_post = requests.patch("{url}/treatment_plans/5irxl".format(url=base_url), headers=auth, json=treatment_plan);
